...
async def stream_file_custom(file_to_stream: BinaryIO, chunk_size=1024):
    max_size = CHANK
    sum_size = 0
    while data_read := await file_to_stream.read(chunk_size):
        yield data_read
        sum_size += chunk_size
...
async def _internal_transfer_to_telegram_custom(client: TelegramClient,
                                         response: BinaryIO,
                                         file_size,
                                         progress_callback: callable
                                         ) -> Tuple[TypeInputFile, int]:
    ...
    callback_size = 0
    # file_size=1024*1024
    if file_size > CHANK: file_size = CHANK

    hash_md5 = hashlib.md5()
    uploader = ParallelTransferrer(client)
    part_size, part_count, is_large = await uploader.init_upload(file_id, file_size)
    buffer = bytearray()
    async for data in stream_file_custom(response):
        if progress_callback:
            callback_size += len(data)

            r = progress_callback(callback_size, file_size)
            if inspect.isawaitable(r):
                await r
        if not is_large:
            hash_md5.update(data)
...
async def download_file_custom(client: TelegramClient,
                        location: TypeLocation,
                        progress_callback: callable = None
                        ) -> BinaryIO:
    callback_size = 0
    ...
    async for x in downloaded:
        ...
        if progress_callback:
            r = progress_callback(callback_size, size)
            if inspect.isawaitable(r):
                await r
...
async def upload_file(client: TelegramClient,
    ...
    res = (await _internal_transfer_to_telegram_custom(client, file, file_size, progress_callback))[0]
    return res